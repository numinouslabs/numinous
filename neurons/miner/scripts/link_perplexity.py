...
def link_perplexity_impl(
    ...
    console.print(
        Panel.fit(
            "[bold cyan]🔗 Link Perplexity API Key[/bold cyan]",
            border_style="cyan",
            padding=(1, 2),
        )
    )
    ...
    console.print(
        Panel.fit(
            ...
            "[dim]Get your API key from:[/dim] [cyan]https://www.perplexity.ai/settings/api[/cyan]\n\n"
            "[yellow]Budget:[/yellow]\n"
            "  • Miner-paid: [green]Your Perplexity usage costs[/green]\n"
    ...
    api_key = getpass("Enter your Perplexity API key: ")
    ...
    if not api_key.startswith("pplx-"):
        console.print("[red]✗[/red] Invalid Perplexity API key format (must start with 'pplx-')")
        raise click.Abort()
    ...
    if not success:
        ...
        console.print(
            Panel.fit(
                f"[red]✗ Failed to link Perplexity API key[/red]\n\n"
    ...
    console.print(
        Panel.fit(
            "[bold green]✓ Successfully linked Perplexity API key![/bold green]\n\n"
            f"[dim]Hotkey:[/dim] {hotkey_keypair.ss58_address[:16]}...\n"
            f"[dim]Service:[/dim] Perplexity\n"
            f"[dim]Budget:[/dim] Miner-paid\n\n"
            "[yellow]Your agent runs will now use your Perplexity API key[/yellow]",
            border_style="green",
            padding=(1, 2),
        )
    )
