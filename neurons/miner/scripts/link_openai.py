...
def link_openai_impl(
    ...
    console.print(
        Panel.fit(
            "[bold cyan]🔗 Link OpenAI API Key[/bold cyan]",
            border_style="cyan",
            padding=(1, 2),
        )
    )
    ...
    console.print(
        Panel.fit(
            ...
            "[dim]Get your API key from:[/dim] [cyan]https://platform.openai.com/api-keys[/cyan]\n\n"
            "[yellow]Budget:[/yellow]\n"
            "  • Miner-paid: [green]Your OpenAI usage costs[/green]\n"
    ...
    with console.status("[cyan]Storing credentials in backend...[/cyan]"):
        success, error_msg = _store_openai_credentials(env, hotkey_keypair, api_key)

    if not success:
        console.print()
        console.print(
            Panel.fit(
                f"[red]✗ Failed to link OpenAI API key[/red]\n\n"
    ...
    console.print(
        Panel.fit(
            "[bold green]✓ Successfully linked OpenAI API key![/bold green]\n\n"
            f"[dim]Hotkey:[/dim] {hotkey_keypair.ss58_address[:16]}...\n"
            f"[dim]Service:[/dim] OpenAI\n"
            f"[dim]Budget:[/dim] Miner-paid\n\n"
            "[yellow]Your agent runs will now use your OpenAI API key[/yellow]",
            border_style="green",
            padding=(1, 2),
        )
    )
