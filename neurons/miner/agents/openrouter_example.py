...
Consider:
1. Historical data or precedents for similar events
...
async def forecast_with_openrouter(event: AgentData) -> dict:
    global TOTAL_COST
    print("[FORECAST] Generating forecast with OpenRouter...")

    messages = build_forecast_messages(event)

    for i, model in enumerate(OPENROUTER_MODELS):
        print(f"[FORECAST] Trying model {i+1}/{len(OPENROUTER_MODELS)}: {model}")
...
async def run_agent(event: AgentData) -> dict:
    ...
    result = await forecast_with_openrouter(event)
