...
def mock_docker_client() -> MagicMock:
    """Ensure docker.from_env().ping() does not require a real daemon."""
    with patch("neurons.validator.utils.env.docker.from_env") as mock_from_env:
...
class TestEnv:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ((3, 11, 0), True),  # Minimum version - should pass
            ((3, 12, 0), True),  # Higher version - should pass
            ((3, 10, 9), False),  # Too low version - should fail
    ...
    def test_docker_daemon_check(self, mock_docker_client: MagicMock):
        mock_docker_client.ping.side_effect = DockerException("Test error")
