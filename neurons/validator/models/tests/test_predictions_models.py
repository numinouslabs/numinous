...
class TestPredictionsModel:
    ...
    def test_create_with_agent_fields(self):
        model = PredictionsModel(
            unique_event_id="event456",
            miner_uid=42,
            miner_hotkey="agent_hotkey",
            latest_prediction=0.85,
            interval_start_minutes=50,
            interval_agg_prediction=0.85,
            run_id="run_abc123_event456_1234567890",
            version_id="version_xyz789",
        )
        assert model.run_id == "run_abc123_event456_1234567890"
        assert model.version_id == "version_xyz789"

    def test_agent_fields_optional(self):
        model = PredictionsModel(
            unique_event_id="event789",
            miner_uid=10,
